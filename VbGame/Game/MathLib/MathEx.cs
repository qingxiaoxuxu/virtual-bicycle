...
namespace VirtualBicycle.MathLib
{
    ...
    public static class MathEx
    {
        ...
        {
            return (bb.Min.X <= p.X & p.X <= bb.Max.X &
                    bb.Min.Y <= p.Y & p.Y <= bb.Max.Y &
                    bb.Min.Z <= p.Z & p.Z <= bb.Max.Z);
        }
        ...
        {
            return ((aabb.Min.X - bs.Radius <= bs.Center.X) & (bs.Center.X <= aabb.Max.X + bs.Radius) &
                    (aabb.Min.Y - bs.Radius <= bs.Center.Y) & (bs.Center.Y <= aabb.Max.Y + bs.Radius) &
                    (aabb.Min.Z - bs.Radius <= bs.Center.Z) & (bs.Center.Z <= aabb.Max.Z + bs.Radius));
        }
        ...
        {
            return ((a.Min.X - bs.Radius <= bs.Center.X) & (bs.Center.X <= a.Max.X + bs.Radius) &
                    (a.Min.Y - bs.Radius <= bs.Center.Y) & (bs.Center.Y <= a.Max.Y + bs.Radius) &
                    (a.Min.Z - bs.Radius <= bs.Center.Z) & (bs.Center.Z <= a.Max.Z + bs.Radius));
        }
        ...
        public static bool AABBIntersects(ref BoundingBox aabb, ref Vector3 vStart, ref Vector3 vEnd)
        {
            ...
            //先检查在盒子内
            if (vStart.X >= aabb.Min.X & vStart.Y >= aabb.Min.Y & vStart.Z >= aabb.Min.Z &
                vStart.X <= aabb.Max.X & vStart.Y <= aabb.Max.Y & vStart.Z <= aabb.Max.Z)
                return true;
            if (vEnd.X >= aabb.Min.X & vEnd.Y >= aabb.Min.Y & vEnd.Z >= aabb.Min.Z &
                vEnd.X <= aabb.Max.X & vEnd.Y <= aabb.Max.Y & vEnd.Z <= aabb.Max.Z)
                return true;

            //依次检查各面的相交情况
            if (vStart.X < aabb.Min.X && vDir.X > 0)
            {
                t = (aabb.Min.X - vStart.X) / vDir.X;
                if (t > 0)
                {
                    vHit = vStart + vDir * t;
                    if (vHit.Y >= aabb.Min.Y && vHit.Y <= aabb.Max.Y &&
                        vHit.Z >= aabb.Min.Z && vHit.Z <= aabb.Max.Z)
                        return true;

                }
            }

            if (vStart.X > aabb.Max.X && vDir.X < 0)
            {
                t = (aabb.Max.X - vStart.X) / vDir.X;
                if (t > 0)
                {
                    vHit = vStart + vDir * t;
                    if (vHit.Y > aabb.Min.Y && vHit.Y <= aabb.Max.Y &&
                        vHit.Z >= aabb.Min.Z && vHit.Y <= aabb.Max.Z)
                        return true;
                }
            }

            if (vStart.Y < aabb.Min.Y && vDir.Y > 0)
            {
                t = (aabb.Min.Y - vStart.Y) / vDir.Y;
                if (t > 0)
                {
                    vHit = vStart + vDir * t;
                    if (vHit.X >= aabb.Min.X && vHit.X <= aabb.Max.X &&
                        vHit.Z >= aabb.Min.Z && vHit.Z <= aabb.Max.Z)
                        return true;
                }
            }

            if (vStart.Y > aabb.Max.Y && vDir.Y < 0)
            {
                t = (aabb.Max.Y - vStart.Y) / vDir.Y;
                if (t > 0)
                {
                    vHit = vStart + vDir * t;
                    if (vHit.X >= aabb.Min.X && vHit.X <= aabb.Max.X &&
            ...
            if (vStart.Z < aabb.Min.Z && vDir.Z > 0)
            {
                t = (aabb.Min.Z - vStart.Z) / vDir.Z;
                if (t > 0)
                {
                    vHit = vStart + vDir * t;
                    if (vHit.X >= aabb.Min.X && vHit.X <= aabb.Max.X &&
                        vHit.Y >= aabb.Min.Y && vHit.Y <= aabb.Max.Y)
                        return true;
                }
            }

            if (vStart.Z > aabb.Max.Z && vDir.Z < 0)
            {
                t = (aabb.Max.Z - vStart.Z) / vDir.Z;
                if (t > 0)
                {
                    vHit = vStart + vDir * t;
                    if (vHit.X >= aabb.Min.X && vHit.X <= aabb.Max.X &&
        ...
        public static bool AABBIntersects(ref BoundingBox aabb, ref Triangle t)
        {
            ...
            return ((aabb.Min.X - r <= center.X) & (center.X <= aabb.Max.X + r) &
                    (aabb.Min.Y - r <= center.Y) & (center.Y <= aabb.Max.Y + r) &
                    (aabb.Min.Z - r <= center.Z) & (center.Z <= aabb.Max.Z + r));
